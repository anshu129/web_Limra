...
public class PlotPrimaryKeyGenerator implements IdentifierGenerator {
    @Override
    public Serializable generate(SharedSessionContractImplementor session, Object object) {
        Connection connection = session.connection();
        try {
            Long nextPrimaryKeyValue = getNextPrimaryKeyValueFromDatabase(connection);
            String formattedPrimaryKey = String.format("PLT%06d", nextPrimaryKeyValue);
    ...
    private Long getNextPrimaryKeyValueFromDatabase(Connection connection) throws SQLException {
        String query = "SELECT MAX(CAST(SUBSTRING(plot_id, 4) AS UNSIGNED)) FROM plot";
